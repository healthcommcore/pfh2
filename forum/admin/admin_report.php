...
{
	case 'config':
		$sql = "SELECT * FROM " . CONFIG_TABLE . " WHERE config_name LIKE 'report_%'";
		...
		$template->assign_vars(array(
			...
			'L_SETTINGS' => $lang['Report_settings'],
	...
	case 'auth':
		if ( isset($HTTP_POST_VARS['submit']) && isset($HTTP_POST_VARS['type']) &&  isset($HTTP_POST_VARS['ug']) && isset($HTTP_POST_VARS['action']) )
		{
			...
			else
			{
				...
				{
					$sql = "SELECT user_id FROM " . USERS_TABLE . " WHERE user_id <> -1 AND username = '$id'";
					if (!$result = $db->sql_query($sql))
					{
			...
			$sql = "UPDATE " . CONFIG_TABLE . " SET config_value = '$new_auth' WHERE config_name = '$auth'";
			if ( !$db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, "Could not update $auth!", "", __LINE__, __FILE__, $sql);
			}
			message_die(GENERAL_MESSAGE, $lang['Report_auth_updated'] . '<br /><br />' . sprintf($lang['Click_return_report_auth'], '<a href="' . append_sid("admin_report.$phpEx?mode=auth&type=" . $HTTP_POST_VARS['type'] . "&ug=" . $HTTP_POST_VARS['ug'] ) . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_report_auth_select'], '<a href="' . append_sid("admin_report.$phpEx?mode=auth") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid("index.$phpEx?pane=right") . '">', '</a>') );
		}
		else
		{
			if ( isset($HTTP_GET_VARS['ug']) && isset($HTTP_GET_VARS['type']) )
			{
				...
				if ( $ug )
				{
					...
					{
						$sql = "SELECT group_id, group_name FROM " . GROUPS_TABLE . " WHERE group_id IN ($board_config[$auth]) ORDER BY group_name ASC";
						if (!$result = $db->sql_query($sql))
						{
					...
					{
						$ug_select .= '<option value="-1">' . $lang['Report_no_groups'] . '</option>';
						$where_sql = '';
					}
					$ug_select .= '</select>';

					$group_select = '<select name="id">';
					$sql = "SELECT group_id, group_name FROM " . GROUPS_TABLE . " WHERE group_single_user <> 1 $where_sql ORDER BY group_name ASC";
					if (!$result = $db->sql_query($sql))
					{
				...
				else
				{
					...
					if ( !empty($board_config[$auth]) )
					{
						$sql = "SELECT user_id, username FROM " . USERS_TABLE . " WHERE user_id IN ($board_config[$auth]) ORDER BY username ASC";
						if (!$result = $db->sql_query($sql))
						{
							message_die(GENERAL_ERROR, "Could not get $auth!", "", __LINE__, __FILE__, $sql);
						}
					...
					{
						$ug_select .= '<option value="-1">' . $lang['Report_no_users'] . '</option>';
					}
					$ug_select .= '</select>';
					$group_select = '';
				}
				...
				$template->assign_vars(array(
					'L_TITLE' => $lang['Report_permissions'] . ' - ' . $lang[ 'Report_' . ( $type ? 'disable' : 'no_auth') ] . ' - ' . $lang[ $ug ? 'Groups' : 'Users' ],
			...
			else
			{
				...
				$template->assign_vars(array(
					'L_TITLE' => $lang['Report_permissions'],
					'L_EXPLAIN' => $lang['Report_permissions_explain'],

					'L_MANAGE_USERS' => $lang['Report_manage_user'],
					'L_MANAGE_GROUPS' => $lang['Report_manage_group'],
					'L_NO_AUTH' => $lang['Report_no_auth'],
					'L_NO_AUTH_EXPLAIN' => $lang['Report_no_auth_explain'],
					'L_DISABLE' => $lang['Report_disable'],
					'L_DISABLE_EXPLAIN' => $lang['Report_disable_explain'],
