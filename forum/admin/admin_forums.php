...
//mb sf1
if (isset($HTTP_POST_VARS['static_toggle']))
{
	$mode = 'change_statics';
}
...
//mb sf1 BEGIN
		case 'change_statics':
		{
			...
			while($row = $db->sql_fetchrow($result))
			{
				//Loop through and update the statics of each forum
				$update_static = (isset($HTTP_POST_VARS['static_id_'.$row['forum_id']])) ? 1 : 0;
				if ($update_static)
				{
					static_sync($row['forum_id']);
				}
				$sql = 'UPDATE ' . FORUMS_TABLE . "
			   SET forum_static = $update_static
			   WHERE forum_id = " . $row['forum_id'];
				if(!$db->sql_query($sql))
				{
					message_die(GENERAL_ERROR, "Could not change forums information", "", __LINE__, __FILE__, $sql);
				}
...
//mb sf1 2lines
				'FORUM_ID' => $forum_id,
				'CHECKED' => ($forum_rows[$j]['forum_static']) ? 'checked="checked"' : '',
