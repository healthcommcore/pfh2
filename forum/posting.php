...
//mb reply entry point
	else if ( $mode == 'reply' )
	{
		$user_sig = ( $userdata['user_sig'] != '' ) ? $userdata['user_sig'] : '';

		$username = ( $userdata['session_logged_in'] ) ? $userdata['username'] : '';
//mb tDefault Subject on Reply
...
//mb don't show announce option in mod/admin post topic
/*
...
/* 
$template->assign_vars(array(
//	'U_VIEWTOPIC' => ( $mode == 'reply' ) ? append_sid("viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id") : '', 
	'U_VIEWTOPIC' => (( $mode == 'editpost' ) || ( $mode == 'reply' )) ? append_sid("viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id") : '', 
    'BREADCRUMB' => " / <a href=\"viewtopic.php?t=".$topic_id."\">".$topic_title."</a>")
);
 */
...
switch( $mode )
{
	case 'newtopic':
		...
		break;
//mb reply redirect to alert was sent, then back to where you were -prvmsg
	case 'reply':
...
//mb no subject if case reply
	// Enable/disable extensions in posting_body
	//
