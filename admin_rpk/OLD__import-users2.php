...
if ($HTTP_POST_VARS["submitAction"]=="insertRecord2") {
    $fieldCount=0;
    while (list($k,$v)=each($HTTP_POST_VARS)) {
        if ($k!="submitAction") {
            if ($fieldCount>0) {
    ...
    $sql="INSERT INTO PFHMailingInfo (".$fieldsList.") VALUES (".$valsList.")";
...
while (list($k,$v)=each($allLines1)) {
    if (strlen(trim($v))>0) {
        ...
        if ($parseLines>0) {
            ...
            if ($allFields1[$k][96]==1) {
                if (!$allUsers[$allFields1[$k][0]]) {
                    echo "<span  style=\"color:#990066; font-size:9pt;\">Study ID: ".$allFields1[$k][1].", &nbsp;Site ID: ".$allFields2[$k][8]."<br>";
                    echo "<b>WEB INTERVENTION</b></span><br><div style=\"color:#990066; padding-left:10px; padding-top:5px;\">";
                ...
                if ($allFields1[$k][1]!="ID") {
                    if (!$allUsers[$allFields1[$k][0]]) {
                        echo "Study ID: ".$allFields1[$k][1].", &nbsp;Site ID: ".$allFields2[$k][8]."<br>";
                        echo "<b><i>PRINT INTERVENTION</i></b><br><div style=\"color:#666699; padding-left:10px; padding-top:5px;\">";
                        echo $allFields2[$k][0]." ".$allFields2[$k][1]."<br>\n";
                        if (strlen(trim($allFields2[$k][2]))>0) echo $allFields2[$k][2]."<br>\n";
                        if (strlen(trim($allFields2[$k][3]))>0) echo $allFields2[$k][3]."<br>\n";
                        echo $allFields2[$k][4].", ".$allFields2[$k][5]." ".$allFields2[$k][6]."<br>\n";
                        echo "</div>\n";
                        echo "<form name=\"form".$allFields1[$k][1]."\" method=\"POST\" action=\"import-users2.php\">\n";
                        echo "<input type=\"hidden\" name=\"submitAction\" value=\"insertRecord2\">\n";
                        echo "<input type=\"hidden\" name=\"webID\" value=\"".$allFields2[$k][7]."\">\n";
                        echo "<input type=\"hidden\" name=\"studyID\" value=\"".$allFields1[$k][1]."\">\n";
                        echo "<input type=\"hidden\" name=\"siteID\" value=\"".$allFields2[$k][8]."\">\n";
                        echo "<input type=\"hidden\" name=\"firstName\" value=\"".$allFields2[$k][0]."\">\n";
                        echo "<input type=\"hidden\" name=\"lastName\" value=\"".$allFields2[$k][1]."\">\n";
                        echo "<input type=\"hidden\" name=\"address1\" value=\"".$allFields2[$k][2]."\">\n";
                        echo "<input type=\"hidden\" name=\"address2\" value=\"".$allFields2[$k][3]."\">\n";
                        echo "<input type=\"hidden\" name=\"city\" value=\"".$allFields2[$k][4]."\">\n";
                        echo "<input type=\"hidden\" name=\"state\" value=\"".$allFields2[$k][5]."\">\n";
                        echo "<input type=\"hidden\" name=\"zip\" value=\"".$allFields2[$k][6]."\">\n";
                        ...
                        echo "Study ID: ".$allFields1[$k][1].", &nbsp;Site ID: ".$allFields2[$k][8]."<br>";
                        echo "<b><i>PRINT INTERVENTION</i></b> (stored in database)<br><div style=\"color:#666699; padding-left:10px; padding-top:5px;\">";
                        echo $allFields2[$k][0]." ".$allFields2[$k][1]."<br>\n";
                        if (strlen(trim($allFields2[$k][2]))>0) echo $allFields2[$k][2]."<br>\n";
                        if (strlen(trim($allFields2[$k][3]))>0) echo $allFields2[$k][3]."<br>\n";
                        echo $allFields2[$k][4].", ".$allFields2[$k][5]." ".$allFields2[$k][6]."<br>\n";
...
<form name="form12">
<input type="button" value="Export PRINT INTERVENTION Mailing Info" style="background:#FFFF00; color:#0066CC; font-size:10pt; font-weight:bold;" onClick="document.location='mailInfo2.php?idSTR=<?=$exportCODE?>';">
</form>
<? } ?>
